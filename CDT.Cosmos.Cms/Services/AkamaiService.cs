...
namespace CDT.Cosmos.Cms.Services
{
    public class AkamaiService
    {
        ...
        {
            _options = options;
            _client = new Client(_options.Value.ClientToken, _options.Value.AccessToken, _options.Value.Secret,
                _options.Value.AkamaiHost);
        }

        public string PurgeCdnByCpCode()
        {
            var purgeObjects = new PurgeObjects {Objects = new[] {_options.Value.CpCode}};
